...
func main() {
	...
	mux.Handle("/ws", auth(http.HandlerFunc(hub.serveWS)))
	...
	reportsFS := http.StripPrefix("/reports/", http.FileServer(http.Dir(researchReportsDir())))
	mux.Handle("/reports/", auth(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Cache-Control", "private, no-store")
		reportsFS.ServeHTTP(w, r)
	})))
...
// guestBlockedMethods 访客禁用的方法：配置/密钥/账号类变更。其余(行情/持仓/AI问答等)照常。
...
func newRPCHandler(app *App, appCtx context.Context) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 访客权限:黑名单方法 403;GetConfig 换成脱敏版
		if role == "guest" {
...
<div class="card">
  ...
  <div class="steps">
    <b>安装步骤(macOS)</b><br>
    1. 下载后解压,得到「JOEY分发包」文件夹<br>
    2. 双击运行 <b>安装.command</b>(首次可能需在系统设置里允许)<br>
    3. 打开 JOEY,<b>注册账号</b>或使用管理员发给你的账号登录即可
  </div>
</div>
</body>
</html>`
